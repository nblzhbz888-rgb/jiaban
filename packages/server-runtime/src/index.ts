import type {
  ...
  MetadataEventSource,
  WebSocketEvent,
} from '@jiaban/server-shared/types'
...
export function setupApp(options?: AppOptions): { app: H3, closeAllPeers: () => void } {
  ...
  const appLogger = useLogg('@jiaban/server-runtime').withLogLevel(logLevelStringToLogLevelMap[appLogLevel]).withFormat(appLogFormat)
  const logger = useLogg('@jiaban/server-runtime:websocket').withLogLevel(logLevelStringToLogLevelMap[websocketLogLevel]).withFormat(websocketLogFormat)
