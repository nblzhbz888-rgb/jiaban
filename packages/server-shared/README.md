# @jiaban/server-shared

The shared type definitions for all server-side packages of Jiaban.

## Usage

```shell
ni @jiaban/server-shared -D # from @antfu/ni, can be installed via `npm i -g @antfu/ni`
pnpm i @jiaban/server-shared -D
yarn i @jiaban/server-shared -D
npm i @jiaban/server-shared -D
```

```typescript
import type { WebSocketEvents } from '@jiaban/server-shared'
```
