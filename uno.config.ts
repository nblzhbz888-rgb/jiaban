...
const DEFAULT_CONTENT_INCLUDE_RE = /\.(vue|svelte|[jt]sx|mdx?|astro|elm|php|phtml|html)($|\?)/
const NODE_MODULES_PATH_RE = /\/node_modules\//
const MASK_RULE_RE = /^mask-\[(.*)\]$/
const BG_DOTTED_RULE_RE = /^bg-dotted-\[(.*)\]$/
const DRAG_REGION_RULE_RE = /drag-region/
...
export function sharedUnoConfig() {
  return defineConfig({
    ...
    rules: [
      ...
      [DRAG_REGION_RULE_RE, () => ({ 'app-region': 'drag' })],
    ],
    theme: {
      colors: {
